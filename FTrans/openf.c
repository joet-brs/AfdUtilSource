...
FcCloseFile(IN HANDLE hFile)
{
    ...
    // 
	OutputDebugString( "Scanning to Close ");
    for ( pFileScan =  g_pOpenFiles; 
         NULL != pFileScan; 
         pFileScan = pFileScan->pNext) {
        ...
        if ( hFile == pFileScan->hFile) {
			...
			OutputDebugString(pFileScan->rgchFile);
			OutputDebugString( "\"");
            ...
            //
            // Free the file when Ref hits '0'
			if (0 == pFileScan->nRefs) {
				OutputDebugString( ", Closing");
	            CloseHandle( pFileScan->hFile);
//				DeleteFile(pFileScan->rgchFile);
				if (g_pOpenFiles == pFileScan) {
					g_pOpenFiles = pFileScan->pNext;
				}
				else {
					for ( pFileScanForDel =  g_pOpenFiles; 
						NULL != pFileScanForDel; 
						pFileScanForDel = pFileScanForDel->pNext) {
						if ( pFileScanForDel->pNext == pFileScan) {
							pFileScanForDel->pNext = pFileScan->pNext;
							break;
						}
					}
				}
