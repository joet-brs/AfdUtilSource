...
BOOL WINAPI VbGetHalbRoot(char *pHalbRoot, DWORD sHalbRoot, DWORD *cbRet) {

	return(GetHalbRoot(pHalbRoot, sHalbRoot, cbRet));
}
...
BOOL WINAPI VbCloseTransactionStruct(transactionControl_t *transactionControlT, transactionInOut_t *transactionInOutT)
{
...
BOOL WINAPI VbInitTransactionStruct(transactionControl_t *transactionControlT, transactionInOut_t *transactionInOutT, char* Service)
//BOOL WINAPI VbInitTransactionStruct(transactionControl_t *transactionControlT, transactionInOut_t *transactionInOutT, HANDLE hDBaccess, SQLHENV* phenv, SQLHDBC* phdbc, SQLHSTMT* phstmt)
{
	return InitTransactionStruct(transactionControlT, transactionInOutT, Service, 0, 0, 0, 0);
}

BOOL WINAPI VbInsertTransactionRecord(transactionControl_t *transactionControlT, transactionInOut_t *transactionInOutT, char* RemoteMachine, char* Identifier, char* Source, char* Destination, long long Size, BOOL Secure, char* Protocols)
{
	transactionControlT->phEnv = &transactionControlT->hEnv;
	transactionControlT->phDBc = &transactionControlT->hDBc;
	transactionControlT->phStmt = &transactionControlT->hStmt;
...
BOOL WINAPI VbUpdateTransactionRecord(transactionControl_t *transactionControlT, transactionInOut_t *transactionInOutT, BOOL complete, long long Size, UDWORD Unused, UDWORD result)
{
	transactionControlT->phEnv = &transactionControlT->hEnv;
	transactionControlT->phDBc = &transactionControlT->hDBc;
	transactionControlT->phStmt = &transactionControlT->hStmt;
	return UpdateTransactionRecord(transactionControlT, transactionInOutT, complete, Size, Unused, result);
}

//BOOL WINAPI VbUpdateTransactionRecordAll(transactionControl_t *transactionControlT, transactionInOut_t *transactionInOutT, char* Source, BOOL complete, long long Size, UDWORD Unused, UDWORD result);
						  ...
						  DWORD *cbLoginRet, DWORD *cbPasswdRet) 
{
	...
	memset(keyPath64, 0, 120);
	strcpy(keyPath64, "SOFTWARE\\Wow6432Node\\MOMENTUM\\Intelligent Network Gateway\\SQL");
	
	/* 
	   SCR 3694:
	   Server Selector cannot write to HKLM hive.
	   AfdAdmin will now search HKCU for the SQL key.
	   If the SQL key isn't there, then search HKLM.
	*/
	...
	if (ret !=0) {
		...
		ret = RegOpenKeyEx(HKEY_CURRENT_USER,		//handle of openkey
						   keyPath64,				//address of name of sub-key to open
						   0,						//reserved
						   KEY_QUERY_VALUE,			//type of access desired
						   &keyHandle				//returned handle
						  );
